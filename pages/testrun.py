...
def preprocess_data(X, fit_transform=False):
    ...
    X = pd.DataFrame(imputer.fit_transform(X) if fit_transform else imputer.transform(X), columns=X.columns)
    
    # Polynomial features
    poly_transformer = PolynomialFeatures(degree=2, include_bias=False, interaction_only=True)
    X_poly = pd.DataFrame(poly_transformer.fit_transform(X[selected_columns]) if fit_transform else poly_transformer.transform(X[selected_columns]), 
                          columns=poly_transformer.get_feature_names_out())
    
    # Drop original columns and add polynomial features
    X = X.drop(columns=selected_columns)
    X = pd.concat([X, X_poly], axis=1)
    
    # Scaling
    scaler = StandardScaler()
    X = pd.DataFrame(scaler.fit_transform(X) if fit_transform else scaler.transform(X), columns=X.columns)
    return X

X_preprocessed = preprocess_data(X, fit_transform=True)
...
base_models = [
    ...
    ('mlp', MLPClassifier(hidden_layer_sizes=(100,), alpha=0.01, max_iter=2000, random_state=20))
]
